#!/usr/bin/env python3
"""Round-9: BECOME the first requester. charts stopped; probe keeps retrying 010C every 20s.
When the ELM device reopens (Hudiy retries), the FIRST query wins the ObdManager slot."""
...
out = {}
alive = True
# Phase 1: wait for ELM (max 25 min), retrying 010C every 20s
print("waiting for ELM device open (max 25min)...", flush=True)
first_ok = None
for i in range(75):
    ...
    if got:
        ...
        break
    print(f"attempt {i+1}: no answer", flush=True)
    time.sleep(5)
...
if first_ok is not None:
    ...
    out["note"] = "ELM never opened in 25min window"

stop.set()
try: client.disconnect()
except Exception: pass
json.dump(out, open("/tmp/diag_probe9.json", "w"), indent=1)
print("WROTE /tmp/diag_probe9.json", flush=True)
