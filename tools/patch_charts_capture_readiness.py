...
new_block = (
    ...
    "        try:" + NL +
    "            if message.request_code >= 930000 and message.result:" + NL +
    '                with open("/tmp/capture_rd.jsonl", "a") as f:' + NL +
...
cap = (
    NL +
    'CAPTURE_PIDS = ["0101", "0141"]' + NL + NL +
    "def _capture_thread():" + NL +
    "    deadline = time.monotonic() + 240" + NL +
    "    cycle = 0" + NL +
    "    while time.monotonic() < deadline and cycle < 4:" + NL +
    "        cycle += 1" + NL +
    "        base = 930000 + cycle * 10" + NL +
...
open("/opt/hudiy-obd-charts/charts_capture_rd.py", "w").write(src)
print("PATCHED-RD")
