import sys
src = open("/opt/hudiy-obd-charts/charts.py").read()
NL = chr(10)
...
new_block = (
    ...
    "        try:" + NL +
    "            if message.request_code >= 920000 and message.result:" + NL +
    '                with open("/tmp/capture_m02.jsonl", "a") as f:' + NL +
...
cap = (
    NL +
    'CAPTURE_PIDS = ["0200", "0202", "0140", "0160", "0180", "01A0"]' + NL + NL +
    "def _capture_thread():" + NL +
    "    deadline = time.monotonic() + 480" + NL +
    "    cycle = 0" + NL +
    "    while time.monotonic() < deadline and cycle < 6:" + NL +
    "        cycle += 1" + NL +
    "        base = 920000 + cycle * 10" + NL +
    ...
    "            except Exception:" + NL +
    "                ok = False" + NL +
    '            with open("/tmp/capture_m02.log", "a") as f:' + NL +
    '                f.write(str(time.time()) + " send " + pid + " req=" + str(req_id) + " ok=" + str(ok))' + NL +
    '                f.write(chr(10))' + NL +
    "            _shutdown_event.wait(2.5)" + NL +
    "        _shutdown_event.wait(8.0)" + NL +
    '    with open("/tmp/capture_m02.log", "a") as f:' + NL +
...
open("/opt/hudiy-obd-charts/charts_capture_m02.py", "w").write(src)
print("PATCHED-M02")
