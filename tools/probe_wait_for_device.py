#!/usr/bin/env python3
"""Round-10: charts dead from BEFORE device-open. Probe connects + waits for 'device opened'.
Auto-detect via ObdManager log? Simpler: keep querying; first answer after open wins."""
...
def send_query(cmd, code, timeout=15.0):
...
out = {}
print("probe10: waiting for device-open (charts dead since boot)...", flush=True)
first_ok = None
for i in range(60):  # up to ~18 min
    got, data = send_query("010C", 9000)
    if got:
        first_ok = data
        print(f"LIVE at attempt {i+1}: {str(data)[:60]}", flush=True)
        out["010C"] = {"answered": True, "raw": data}
        break
    if i % 5 == 0:
        print(f"attempt {i+1}: waiting", flush=True)
    time.sleep(3)
...
json.dump(out, open("/tmp/diag_probe10.json", "w"), indent=1)
print("WROTE /tmp/diag_probe10.json", flush=True)
