...
**Update 2026-09-14 (input):** the wheel/knob adapter now speaks Hudiy's full navigation scheme - keys `1`/`2` (scroll left/right), arrow keys, `enter`, `escape`, plus mouse-style wheel encoders - with device auto-discovery (`--scan` shows candidates on any machine). See `docs/HUDIY_KEYBOARD_CONTROL_SCHEME.md` section 4.
